...
module.exports = async (options) => {
	...
	if (parseInt(options.time) < 10000) {
		throw new Error(
			'Error: time argument must be greater than 10 Seconds (in ms i.e. 10000).',
		);
	}
	if (typeof options.time !== 'number') {
		throw new TypeError('Error: time must be a number.');
	}
	...
	if (typeof options.waitMessage !== 'string') {
		throw new TypeError('Error: waitMessage must be a string.');
	}
	...
	if (typeof options.startMessage !== 'string') {
		throw new TypeError('Error: startMessage must be a string.');
	}
	...
	if (typeof options.emoji !== 'string') {
		throw new TypeError('Error: emoji must be a string.');
	}
	...
	if (typeof options.ongoingMessage !== 'string') {
		throw new TypeError('Error: ongoingMessage must be a string.');
	}
